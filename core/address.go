...
// Address design of dacc address
/*
[Account Address]
Similar to Bitcoin and Ethereum, Dacc also adopts elliptic curve algorithm as its basic encryption algorithm for Dacc accounts.
...
The final address of Dacc Wallet should be:  n1TV3sU6jyzR4rJ1D7jCAmtVGSntJagXZHC
...
For more information, plz check (https://github.com/daccproject/wiki/blob/master/tutorials/%5BEnglish%5D%20Dacc%20101%20-%2003%20Smart%20Contracts%20JavaScript.md) and [rpc.sendTransaction](https://github.com/daccproject/wiki/blob/master/rpc.md#sendtransaction).
...
Addition of extended information allows the addition of another element verification to the Dacc Wallet APP. For example:
...
func AddressParse(s string) (*Address, error) {
	if len(s) != AddressBase58Length || s[0] != DaccFaith {
